...
update_repository_urls() {
    ...
    echo "  find . -name '*.yaml' -type f -exec sed -i 's|mlworkbench-com|dverdonschot|g' {} +"
    echo ""
    echo "Or add your git repository to ArgoCD via CLI:"
    echo ""
    echo "  argocd repo add https://github.com/dverdonschot/mlworkbench.git \\"
    echo "    --username dverdonschot \\"
